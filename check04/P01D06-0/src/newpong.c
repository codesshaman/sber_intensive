...
int num_sc_char = 55;
int winscore = 21;
void moveRocket(int, int); //объявление функции для движения ракеток по оси Y
void moveBall(int, int, int, int, int, int); //объявление функции для движения мяча: по осям X, Y; переменные для задания вектора движения; переменные для задания положения ракеток по оси Y,
